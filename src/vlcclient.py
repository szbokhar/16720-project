...
class VLCClient(object):
    ...
    def connect(self):
        ...
        result = self.telnet.expect([
            b"VLC media player ([\d.]+)",
        ])
        self.server_version = result[1].group(1)
        self.server_version_tuple = self.server_version.split(b'.')

        # Login
        self.telnet.read_until(b"Password: ")
        self.telnet.write(self.password.encode("ascii"))
        self.telnet.write("\n".encode("ascii"))
    ...
    def _send_command(self, line):
        ...
        return self.telnet.read_until(b">")[1:-3]
